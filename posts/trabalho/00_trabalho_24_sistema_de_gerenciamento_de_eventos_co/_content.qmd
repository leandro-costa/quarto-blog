# Trabalho 24 – Sistema de Gerenciamento de Eventos Corporativos – Agenda e Convites
...
|--------|----------------|
| **Apresentação (JavaFX)** | Tela de cadastro de eventos, tela de gestão de convites (enviar/cancelar), tela de agenda (visão cronológica) e tela de listagem de participantes. Cada tela deve exibir mensagens de erro claras. |
| **Negócio** | • Validar dados de entrada (nome do evento, data, capacidade máxima, e-mail do convidado). <br>• Aplicar as regras de negócio descritas abaixo. |
| **Dados** | • Armazenar eventos e convites em memória (`ArrayList`, `HashMap`). <br>• Implementar operações CRUD para eventos e convites. |
...
3. O serviço verifica as regras de negócio (conflito de data e capacidade). Se válidas, delega ao **Repositório de Eventos** (camada de dados). Se violadas, retorna um erro.
...
| **Camada de Negócio** | 30 pts | Implementação correta das regras de conflito e capacidade, tratamento de exceções. |
